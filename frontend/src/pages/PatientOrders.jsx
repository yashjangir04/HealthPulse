...
import { getPatientOrders, acceptOrder } from "../api/order";
...
const PatientOrders = () => {
  ...
  const [contactDetails, setContactDetails] = useState({ shopName: "", phone: "" });
  ...
  const handleAcceptOffer = async (orderId, resp) => {
    ...
    try {
      const targetShopkeeperId = resp?.shopkeeper?._id || resp?.shopkeeperID?._id || resp?.shopkeeperID;
      ...
      const shopName = resp?.shopkeeper?.shopName || resp?.shopkeeperID?.shopName || "Local Pharmacy";
      const phone = resp?.shopkeeper?.phoneNumber || resp?.shopkeeperID?.phoneNumber || "+91 XXXXXXXXXX"; 
  ...
  const handleRejectOffer = async (orderId, responseId) => {
    ...
    try {
      ...
      setOrders(prev => prev.map(order => {
        if (order._id === orderId) {
          const updatedResponses = order.responses.map(r => 
            r._id === responseId ? { ...r, status: "rejected" } : r
          );
          return { ...order, responses: updatedResponses };
        }
        return order;
      }));
    } catch (error) {
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-500 font-medium mt-2 text-[15px] max-w-lg">
            Review your medicine requests, compare offers from local pharmacies, and track your deliveries.
          </p>
        </div>
        ...
        {filteredOrders.length === 0 ? (
          <div className="bg-white rounded-3xl border border-slate-200 border-dashed p-16 flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-1">No {activeTab} orders</h3>
          ...
          <div className="space-y-6">
            {filteredOrders.map((order) => {
              ...
              const winningResponse = order.status === "done" 
              ...
              return (
                <div key={order._id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <div className="p-6 sm:p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        {order.status === "pending" ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-blue-100 text-blue-700 shrink-0">
                            <Activity size={12} className="animate-pulse" /> Awaiting Offers
                          </span>
                        ) : (
                    ...
                    <div className="flex -space-x-2 overflow-hidden sm:justify-end">
                      {safeMedicines.slice(0, 3).map((med, idx) => {
                        ...
                        return (
                          <div key={med._id || idx} className="inline-block h-10 w-10 rounded-full ring-2 ring-white bg-blue-50 items-center justify-center text-[10px] font-bold text-blue-600 border border-blue-100" title={medName}>
                  ...
                  {winningResponse && (
                    <div className="px-6 py-4 bg-blue-50/50 border-b border-blue-100 flex items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-bold text-blue-600 uppercase tracking-wider">Fulfilled By</p>
                          <p className="font-black text-slate-900 text-sm">
                            {winningResponse?.shopkeeper?.shopName || winningResponse?.shopkeeperID?.shopName || "Local Pharmacy"}
                          </p>
                        </div>
                      </div>
                      <a 
                  ...
                  <div className="p-6 sm:p-8 bg-slate-50/30">
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Requested Items ({safeMedicines.length})</h4>
                    <div className="flex flex-wrap gap-2 mb-8">
                      {safeMedicines.map((med, idx) => (
                        <span key={med._id || idx} className="inline-flex items-center px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-sm font-medium text-slate-700 shadow-sm">
                          {med?.name || "Unknown Medicine"} 
                          {med?.dose && <span className="ml-1.5 text-[11px] text-slate-400">{med.dose}</span>}
                        </span>
                      ))}
                    </div>

                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Offers from Pharmacies</h4>
                    
                    {safeResponses.filter(r => r?.status === "pending" || r?.status === "accepted").length === 0 ? (
                      <div className="text-center py-8 bg-white rounded-2xl border border-dashed border-slate-200">
                        <p className="text-sm font-medium text-slate-500">Waiting for pharmacies to respond with their quotes...</p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {safeResponses
                          ...
                          const isAccepted = resp.status === "accepted" && order.status !== "pending";
                          const isBestPrice = idx === 0 && order.status === "pending";
                          
                          const shopName = resp?.shopkeeper?.shopName || resp?.shopkeeperID?.shopName || "Local Pharmacy";
                          const distance = resp?.distance ? `${resp.distance} km` : "Nearby";

                          return (
                            <div key={resp._id} className={`relative p-5 rounded-2xl border-2 transition-all duration-300 flex flex-col ${
                              isAccepted 
                              ...
                              <div className="flex justify-between items-start mb-4">
                                <div className="flex gap-3 items-center">
                                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isAccepted ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                                    <Store size={18} />
                                  </div>
                                  <div>
                                    <h5 className="font-black text-[15px] text-slate-900">{shopName}</h5>
                              ...
                              <div className="flex items-end justify-between mt-auto pt-4 border-t border-slate-100">
                                <div>
                                  <span className="text-[10px] font-bold text-slate-400 uppercase block mb-0.5">Quoted Price</span>
                                  <span className={`text-xl font-black flex items-center ${isAccepted ? 'text-emerald-700' : 'text-slate-900'}`}>
                                ...
                                {order.status === "pending" && (
                                  <div className="flex items-center gap-2">
                                    <button
                                      onClick={() => handleRejectOffer(order._id, resp._id)}
                                    ...
                                    <button
                                      onClick={() => handleAcceptOffer(order._id, resp)}
                                      disabled={processingId !== null}
